...
class TestRotamers(TestCase):
    def test_rotamers(self):
        ...
        rot_s = list(StructureReader(
            '/oak/stanford/groups/rondror/projects/combind/bpp_data/MAPK14/structures/aligned_files/1KV1/1KV1_out.mae'))[
            0]
        rot_m = list(rot_s.molecule)[0]
        rot_r = list(rot_m.residue)[0]
        s = list(StructureReader(
            '/oak/stanford/groups/rondror/projects/combind/bpp_data/MAPK14/structures/aligned_files/1KV1/1KV1_out.mae'))[
            0]
        m = list(s.molecule)[0]
        r = list(m.residue)[0]
        (num_rots, avg_rot_rmsd, num_r_rots, avg_r_rot_rmsd) = residue_feature_vector_creator.rotamers(rot_s, rot_r, s,
                                                                                                       r, 50)
        ...
        s2 = list(StructureReader(
            '/oak/stanford/groups/rondror/projects/combind/bpp_data/MAPK14/structures/aligned_files/1KV1/1KV1_out.mae'))[
            0]
        m2 = list(s2.molecule)[18]
        r2 = list(m2.residue)[18]
        (num_rots2, avg_rot_rmsd2, num_r_rots2, avg_r_rot_rmsd2) = residue_feature_vector_creator.rotamers(rot_s2,
                                                                                                           rot_r2, s2,
                                                                                                           r2, 50)
...
class TestPacking(TestCase):
    def testPacking(self):
        ...
        m = list(s.molecule)[0]
        r1 = list(m.residue)[0]
        r2 = list(m.residue)[82]
        packing_val1 = residue_feature_vector_creator.packing(s, r1)
        packing_val2 = residue_feature_vector_creator.packing(s, r2)

        self.assertEqual(packing_val1 < packing_val2, True)
